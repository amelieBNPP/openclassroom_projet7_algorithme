...
class BrutForce:
    ...
    def brut_force(self):
        ...
        1. compute all possible values for each share, here [0,1] because a share can be bought only one time
        ...
        # 1. compute possibles values for each share
        nb_unknow = len(self.share_price)

        all_values = [[0, price] for price in self.share_price]

        # 2. compute all combinaison
        all_combinaisons = [combinaison for combinaison in product(
            *all_values) if sum(combinaison) <= self.portfolio_capacity]
        ...
        self.best_shares = {
            self.share_name[share_idx]: (
                best_combinaison /
                self.share_price[share_idx]) for share_idx,
            best_combinaison in enumerate(best_shares_list) if best_combinaison != 0}
