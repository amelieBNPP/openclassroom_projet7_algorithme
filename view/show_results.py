...
class View:
    ...
    def __init__(self, input_share_name, portfolio_capacity, algorithm) -> None:
        self.input_share_name = input_share_name
        self.portfolio_capacity = portfolio_capacity
        self.algorithm = algorithm

    def display_results(self, optim_result, optim_time, nb_share_before_treatment):
    ...
    def display_optim(self) -> None:
        print('portfolio profit : ' + str(self.best_profit['Total Profit']) + ' euros\n')
        print('investment :')
        for share in self.best_shares:
            print(' -' + str(int(self.best_shares[share])) + ' share(s) ' + share)
        print('\ninvestment cost : ' + str(self.best_cost['Best cost']) + ' euros')
